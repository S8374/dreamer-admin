        ...
        <div className="flex flex-col gap-4">
          {users.map((user: any, index: number) => {
            ...
            return (
              <div 
                key={user.id} 
                className={`flex flex-col sm:flex-row sm:items-center gap-4 p-4 rounded-xl border transition-all hover:scale-[1.01] hover:shadow-md ${bgColor}`}
              >
                <div className="flex items-center gap-3 w-full sm:w-auto">
                  {/* Rank Badge */}
                  <div className="flex-shrink-0 w-10 sm:w-12 flex flex-col items-center justify-center">
                    {RankIcon ? (
                      <RankIcon className={`h-7 w-7 sm:h-8 sm:w-8 ${iconColor}`} />
                    ) : (
                      <span className="text-lg sm:text-xl font-bold text-zinc-400">#{index + 1}</span>
                    )}
                  </div>
                  ...
                  <div className="relative">
                    <img 
                      ...
                      alt={user.fullName}
                      className={`h-10 w-10 sm:h-12 sm:w-12 rounded-full object-cover ${isTop3 ? 'ring-2 ring-offset-2 ring-[#6b8f84] dark:ring-offset-zinc-900' : 'border border-zinc-200 dark:border-zinc-700'}`}
                    />
                  ...
                  <div className="flex-grow min-w-0">
                    <h3 className="font-bold text-sm sm:text-base text-zinc-900 dark:text-zinc-100 truncate flex items-center gap-2">
                      {user.fullName || "Unnamed User"}
                      {isTop3 && <span className="hidden sm:inline-flex text-[10px] px-2 py-0.5 rounded-full bg-[#6b8f84]/10 text-[#6b8f84] font-medium">Top Rated</span>}
                    </h3>
                    <p className="text-xs sm:text-sm text-zinc-500 truncate max-w-[150px] sm:max-w-xs">{user.email}</p>
                  </div>
                </div>

                <div className="flex items-center justify-between sm:justify-end gap-4 w-full sm:flex-1 sm:px-4 mt-2 sm:mt-0 pt-3 sm:pt-0 border-t sm:border-transparent border-zinc-200 dark:border-zinc-800">
                  {/* Stats */}
                  <div className="flex items-center gap-4 sm:gap-6">
                    <div className="flex flex-col items-end">
                      <span className="text-[10px] sm:text-xs font-medium text-zinc-500 uppercase tracking-wider">Rating</span>
                      <div className="flex items-center gap-1">
                        <Star className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-yellow-400 fill-current" />
                        <span className="font-bold text-sm sm:text-base text-zinc-900 dark:text-zinc-100">{user.rating?.toFixed(1) || "0.0"}</span>
                      </div>
                    </div>
                    
                    <div className="flex flex-col items-end">
                      <span className="text-[10px] sm:text-xs font-medium text-zinc-500 uppercase tracking-wider">Reviews</span>
                      <span className="font-bold text-sm sm:text-base text-zinc-900 dark:text-zinc-100">{user.reviewCount || 0}</span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2 sm:gap-4">
                    <div className="hidden sm:block">
                      <span className={`inline-flex items-center px-2 sm:px-3 py-1 rounded-full text-[10px] sm:text-xs font-bold uppercase tracking-wider ${
