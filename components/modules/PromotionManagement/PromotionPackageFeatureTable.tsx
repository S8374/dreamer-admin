...
import { PromotionPackageFeatureModal } from "./PromotionPackageFeatureModal";

export function PromotionPackageFeatureTable({ promotionPackageId }: { promotionPackageId: string }) {
  ...
  const { data, isLoading, isFetching } = useGetPromotionPackageFeaturesQuery(promotionPackageId);

  const [deleteFeature] = useDeletePromotionPackageFeatureMutation();

  const handleDelete = async (featureId: string) => {
    if (!window.confirm("Are you sure you want to delete this feature?")) return;
    try {
      await deleteFeature({ packageId: promotionPackageId, featureId }).unwrap();
