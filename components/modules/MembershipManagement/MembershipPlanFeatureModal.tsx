...
export function MembershipPlanFeatureModal({ isOpen, onClose, feature, membershipPlanId, nextSortOrder }: MembershipPlanFeatureModalProps) {
  ...
  // Auto-generate code from details if creating a new feature and code is empty
  useEffect(() => {
    if (!feature && formData.details && !formData.code) {
      const generatedCode = formData.details
        .toUpperCase()
        .replace(/[^A-Z0-9\s]/g, "")
        .trim()
        .replace(/\s+/g, "_")
            ...
            <div>
              ...
              <div className="flex gap-2">
                ...
                <button
                  type="button"
                    onClick={() => {
                       const baseText = formData.details || "FEATURE_" + Math.random().toString(36).substring(2, 8);
